...
pub fn valueToForm(allocator: Allocator, val: Value) Allocator.Error!Form {
    return switch (val.tag()) {
        ...
        .fn_val, .builtin_fn, .atom, .volatile_ref, .protocol, .protocol_fn, .multi_fn, .delay, .future, .promise, .agent, .reduced, .transient_vector, .transient_map, .transient_set, .chunked_cons, .chunk_buffer, .array_chunk, .wasm_module, .wasm_fn, .matcher, .array => Form{ .data = .nil },
    };
}
