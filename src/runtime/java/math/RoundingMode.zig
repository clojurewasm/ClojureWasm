...
//! `runtime/rounding_mode.zig` (so the analyzer's static-field resolve +
//! `Runtime.deinit` reach them without importing this surface tree, per the zone
//! rule). This file owns the descriptor + static-field table + `.toString`.
...
fn toString(rt: *Runtime, env: *Env, args: []const Value, loc: SourceLocation) anyerror!Value {
    ...
    return string_collection.alloc(rt, rounding_mode.name(ord));
}
...
const static_fields = build: {
    ...
    for (&arr, 0..) |*sf, i| {
        sf.* = .{ .name = rounding_mode.name(@intCast(i)), .value = .{ .rounding_mode = @intCast(i) } };
    }
    break :build arr;
};
