...
(defn walk
  ;; Traverses form applying inner to elements, then outer to result.
  [inner outer form]
  (cond
...
(defn postwalk
  ;; Depth-first post-order traversal, calls f on each sub-form.
  [f form]
...
(defn postwalk-replace
  ;; Replace keys in smap with values throughout form.
  [smap form]
  (postwalk (fn [x] (if (contains? smap x) (get smap x) x)) form))

(defn prewalk-replace
  ;; Replace at root first.
  [smap form]
  (prewalk (fn [x] (if (contains? smap x) (get smap x) x)) form))

;; ========== clojure.template functions ==========

(defn apply-template
  ;; Replace symbols in argv with corresponding values in expr.
  [argv expr values]
  (postwalk-replace (zipmap argv values) expr))

(defmacro do-template
  ;; Repeatedly expand expr for each group of arguments.
  [argv expr & values]
  (let [c (count argv)
