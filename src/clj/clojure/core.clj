...
;; `global-hierarchy` — now initialized by Zig (hierarchy functions look it up from env)
(def ^:private global-hierarchy (make-hierarchy))

;; `isa?` migrated to Zig (collections.zig)
;; `parents` migrated to Zig (collections.zig)
;; `ancestors` migrated to Zig (collections.zig)
;; `descendants` migrated to Zig (collections.zig)
;; `derive` migrated to Zig (collections.zig)
;; `underive` migrated to Zig (collections.zig)
