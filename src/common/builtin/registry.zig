...
test "all_builtins count" {
    // 225 + 1 (alter-var-root) + 2 (prefer-method, prefers) + 1 (sorted-set) + 1 (load-file) + 1 (load)
    try std.testing.expectEqual(231, builtin_count);
}
