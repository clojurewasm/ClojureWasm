...
const PersistentArrayMap = @import("../runtime/collections.zig").PersistentArrayMap;
...
pub fn withMetaFn(allocator: Allocator, args: []const Value) anyerror!Value {
    ...
    return switch (obj.tag()) {
        .list => blk: {
            const l = obj.asList();
            const new_list = try allocator.create(@import("../runtime/collections.zig").PersistentList);
            new_list.* = .{ .items = l.items, .meta = meta_ptr };
            break :blk Value.initList(new_list);
        },
        .vector => blk: {
            const v = obj.asVector();
            const new_vec = try allocator.create(@import("../runtime/collections.zig").PersistentVector);
        ...
        .hash_map => blk: {
            ...
            const new_hm = try allocator.create(@import("../runtime/collections.zig").PersistentHashMap);
            new_hm.* = .{
                .count = hm.count,
                .root = hm.root,
        ...
        .set => blk: {
            const s = obj.asSet();
            const new_set = try allocator.create(@import("../runtime/collections.zig").PersistentHashSet);
            new_set.* = .{ .items = s.items, .meta = meta_ptr, .comparator = s.comparator };
            break :blk Value.initSet(new_set);
        },
        .fn_val => blk: {
            const f = obj.asFn();
            const new_fn = try allocator.create(@import("../runtime/value.zig").Fn);
            new_fn.* = .{
                .proto = f.proto,
                .kind = f.kind,
...
test "meta on vector with no metadata returns nil" {
    ...
    const vec = @import("../runtime/collections.zig").PersistentVector{ .items = &items };
...
test "with-meta on vector attaches metadata" {
    ...
    // Create vector [1 2]
    const items = [_]Value{ Value.initInteger(1), Value.initInteger(2) };
    const vec = try alloc.create(@import("../runtime/collections.zig").PersistentVector);
    vec.* = .{ .items = &items };
    const val = Value.initVector(vec);
...
test "with-meta on list attaches metadata" {
    ...
    const list = try alloc.create(@import("../runtime/collections.zig").PersistentList);
    list.* = .{ .items = &items };
...
test "with-meta on fn_val attaches metadata" {
    ...
    const fn_struct = try alloc.create(@import("../runtime/value.zig").Fn);
