...
fn callProtocolDatafy(allocator: Allocator, x: Value) anyerror!Value {
    const protocol = ns_core_protocols.datafiable_protocol orelse return x;
...
fn callProtocolNav(allocator: Allocator, coll: Value, k: Value, v: Value) anyerror!Value {
    const nav_protocol = ns_core_protocols.navigable_protocol orelse return v;
...
/// We register under the "map" type key — the protocol dispatch will check
/// for __ex_info maps. Since Object already has a fallback (identity), and
/// ex-info maps are just maps, we extend for the "map" type.
/// Note: This is a simplification — upstream extends Throwable.
/// In CW, we check if the map is an ex-info in the implementation function.
pub fn registerDatafyExtensions(allocator: Allocator) !void {
    _ = allocator;
    ...
    // The upstream Throwable extension returns (ex-data x), but in CW
    // ex-info is already a map containing :data, so identity is equivalent
    // to the upstream behavior for practical purposes.
