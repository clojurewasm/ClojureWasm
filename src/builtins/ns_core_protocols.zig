...
/// Called from registerBuiltins() in registry.zig.
pub fn registerProtocols(allocator: Allocator, env: *Env) !void {
    ...
    // Store protocol pointers as module-level for access by ns_datafy.zig
    datafiable_protocol = datafiable;
    navigable_protocol = navigable;
    coll_reduce_protocol = coll_reduce;
}

/// Protocol pointers accessible by other modules (e.g., ns_datafy, collections).
