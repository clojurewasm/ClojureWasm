...
/// CW has no cause chains — ex-cause always returns nil, so this is identity.
fn rootCauseFn(allocator: Allocator, args: []const Value) anyerror!Value {
    ...
    var tr = args[0];
    // Follow ex-cause chain (usually nil in CW)
    while (true) {
...
/// e is a [ns/fn file line] vector from Throwable->map :trace
...
fn printStackTraceFn(allocator: Allocator, args: []const Value) anyerror!Value {
    ...
    // Print remaining trace elements
...
fn printCauseTraceFn(allocator: Allocator, args: []const Value) anyerror!Value {
    ...
    // Iterative version of the recursive .clj (recur cause n)
    while (true) {
        if (n.tag() == .nil) {
