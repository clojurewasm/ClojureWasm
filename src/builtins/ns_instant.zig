...
/// Parse a timestamp string into components using regex.
...
/// Validate timestamp component ranges (from `validated`).
...
/// Construct a Date instance from components via __inst-from-string.
...
/// Convert TimestampComponents to Value args array for callFnVal.
...
/// __validated-call: hidden builtin for (validated constructor).
/// Args: [constructor, years, months, days, hours, minutes, seconds, nanoseconds,
...
fn validatedFn(allocator: Allocator, args: []const Value) anyerror!Value {
    ...
    // Resolve __validated-call builtin, then (partial __validated-call new-instance)
    const env = bootstrap.macro_eval_env orelse return error.EvalError;
