...
# operands, incl. mixed ratio⊗integer, integer-collapse to BigInt N, and
# float-contagion (ratio⊗float). Mirrors JVM Clojure: (+ 1/2 1/2) → 1
# (a BigInt 1N, not 1/1 — clj's Ratio collapse, D-272); (* 1/2 0.5) → 0.25 (float).
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# ratio ⊗ ratio
assert_eq 'add_collapse' "$("$BIN" -e '(+ 1/2 1/2)')"   '1N'
...
assert_eq 'mul_collapse' "$("$BIN" -e '(* 2/3 3/2)')"   '1N'
assert_eq 'div_ratio'    "$("$BIN" -e '(/ 1/2 1/3)')"   '3/2'
# ratio ⊗ integer (mixed)
assert_eq 'mul_int'      "$("$BIN" -e '(* 1/2 4)')"     '2N'
assert_eq 'mul_int_l'    "$("$BIN" -e '(* 4 1/2)')"     '2N'
assert_eq 'mul_int_r'    "$("$BIN" -e '(* 1/10 10)')"   '1N'
assert_eq 'add_int'      "$("$BIN" -e '(+ 3 1/2)')"     '7/2'
assert_eq 'sub_int'      "$("$BIN" -e '(- 1/2 3)')"     '-5/2'
assert_eq 'div_int'      "$("$BIN" -e '(/ 1/2 3)')"     '1/6'
assert_eq 'div_int_l'    "$("$BIN" -e '(/ 3 1/2)')"     '6N'
# integer / by ratio collapse
assert_eq 'mul_chain'    "$("$BIN" -e '(* 1/2 2/3 3/4)')" '1/4'
assert_eq 'add_chain'    "$("$BIN" -e '(+ 1/4 1/4 1/4 1/4)')" '1N'
