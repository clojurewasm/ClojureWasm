...
got="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/rename-keys (hash-map :a 1 :b 2) (hash-map :a :A)))')"
...
# dissoc/assoc).
got2="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/rename-keys (clojure.set/rename-keys (hash-map :a 1 :b 2) (hash-map :a :A)) (hash-map :A :a)))')"
assert_eq 'rename_keys_round_trip' "$got2" '{:b 2, :a 1}'

# rename a key whose source is absent → no-op for that mapping.
got="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/rename-keys (hash-map :a 1) (hash-map :missing :gone)))')"
assert_eq 'rename_keys_absent_source' "$got" '{:a 1}'

# rename to a key that already exists — value should overwrite.
got="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/rename-keys (hash-map :a 1 :b 2) (hash-map :a :b)))')"
assert_eq 'rename_keys_collision' "$got" '{:b 1}'

# empty rename-map → identity
got="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/rename-keys (hash-map :x 9) (hash-map)))')"
assert_eq 'rename_keys_empty_rename' "$got" '{:x 9}'

# --- map-invert ---

got="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/map-invert (hash-map :a 1 :b 2)))')"
# Round-trip via map-invert twice should equal original.
got2="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/map-invert (clojure.set/map-invert (hash-map :a 1 :b 2))))')"
assert_eq 'map_invert_round_trip' "$got2" '{:a 1, :b 2}'

got="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/map-invert (hash-map)))')"
assert_eq 'map_invert_empty' "$got" '{}'

# Sanity-check the inverted key lookups via re-invert.
got="$("$BIN" -e '(do (require (quote [clojure.set])) (clojure.set/map-invert (hash-map :x :y)))')"
