...
#
# clojure.repl's REPL-discovery helpers, provided in cljw's core (alongside
# `doc`): `dir-fn` / `dir` / `apropos`. Upstream clojure.repl is host-coupled
# (source/pst/demunge → JVM stack + Compiler; set-break-handler → sun.misc.Signal),
# so only this no-JVM-feasible subset is implemented. `find-doc` is omitted — cljw
# vars carry no :doc metadata (AD-041). `dir-fn` matches clj exactly; `apropos`
# scans all-ns (the exact qualified-symbol set is ns-set-dependent, so assert
...
ap="$("$BIN" -e '(prn (apropos "reduce-kv"))')"
assert_contains 'apropos_kv' "$ap" 'reduce-kv'
# apropos with a regex.
apr="$("$BIN" -e '(prn (apropos #"^reductions$"))')"
assert_contains 'apropos_re' "$apr" 'reductions'
# apropos returns a (sorted) seq.
assert_eq 'apropos_empty' "$("$BIN" -e '(seq? (apropos "definitely-no-such-var-xyzzy"))')" 'true'
