;; Ported from clojure/test_clojure/clojure_set.clj
;; Author: Frantisek Sodomka (original)
...
(deftest test-union
  (are [x y] (= x y)
    ...
    (set/union #{1 2} #{:a :b} #{nil} #{false true} #{[] [1 2]}
               #{{} {:a 1}} #{#{} #{1 2}})
    #{1 2 :a :b nil false true [] [1 2] {} {:a 1} #{} #{1 2}}

      ;; SKIP: sorted-set / hash-set tests (not implemented)
    ))

(deftest test-intersection
  ;; SKIP: (is (thrown? IllegalArgumentException (set/intersection)))
  ;; intersection requires at least one argument
