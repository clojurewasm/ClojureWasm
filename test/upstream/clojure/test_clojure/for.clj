...
(defn only
  ...
  [n]
  (lazy-cat (range n)
            (throw "consumer went too far in lazy seq"))) ;; CLJW: throw string instead of Exception.
...
(deftest-both While-and-When-Same-Binding
  ...
  ;; CLJW: replaced Math/abs with inline abs, Ratio 1/3 with (/ 1.0 3.0)
  (is (= (for [a (range -2 5)
               :when (not= a 0)
               :while (> (let [x (/ 1.0 a)] (if (neg? x) (- x) x)) (/ 1.0 3.0))] a) '(-2 -1 1 2))))
