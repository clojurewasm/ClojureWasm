...
;; Upstream: spec.alpha/src/test/clojure/clojure/test_clojure/spec.clj
;; Upstream lines: 312
;; CLJW markers: 19
...
(ns clojure.test-clojure.spec
  (:require [clojure.spec.alpha :as s]
            ...
            ;; CLJW: clojure.spec.test.alpha not implemented
            [clojure.test :refer :all]))

;; CLJW: *warn-on-reflection* not applicable in CW
;; (set! *warn-on-reflection* true)

;; CLJW: No Java exception class hierarchy. Catch Exception and return "error" string.
;; UPSTREAM-DIFF: Returns "error" instead of Java class name string.
;; Must be a string (seqable) because test body does (map submap? ed e).
...
(deftest conform-explain
  (let [a (s/and #(> % 5) #(< % 10))
        ...
        ;; CLJW: inst-in stubbed — inst types not yet available
    ...
    (are [spec x conformed ed]
      ...
      lrange 7 7 nil
      lrange 8 8 nil
      ;; CLJW: pred form uses unqualified fn (CW resolve returns nil for fn)
      ;; UPSTREAM-DIFF: (fn [%] ...) instead of (clojure.core/fn [%] ...)
      lrange 42 ::s/invalid [{:pred '(fn [%] (clojure.spec.alpha/int-in-range? 7 42 %)), :val 42}]

      ;; CLJW: inst-in tests skipped — inst types not available
      ...
      ;; CLJW: pred form uses unqualified fn
      drange 3.0 ::s/invalid [{:pred '(fn [%] (clojure.core/<= 3.1 %)), :val 3.0}]
      drange 3.1 3.1 nil
      drange 3.2 3.2 nil
      ;; CLJW: ##Inf instead of Double/POSITIVE_INFINITY; pred uses unqualified fn
      drange ##Inf ::s/invalid [{:pred '(fn [%] (clojure.core/not (Double/isInfinite %))), :val ##Inf}]
      ...
      ;; CLJW: fn-sym returns nil, so plain predicates get ::unknown as form
      ;; UPSTREAM-DIFF: pred is ::s/unknown instead of `keyword?
      keyword? :k :k nil
      keyword? nil ::s/invalid [{:pred ::s/unknown :val nil}]
      keyword? "abc" ::s/invalid [{:pred ::s/unknown :val "abc"}]

      ;; CLJW: pred forms use unqualified fn
      a 6 6 nil
      a 3 ::s/invalid '[{:pred (fn [%] (clojure.core/> % 5)), :val 3}]
      a 20 ::s/invalid '[{:pred (fn [%] (clojure.core/< % 10)), :val 20}]
      ;; CLJW: returns "error" instead of Java exception class name
      a nil "error" "error"
      a :k "error" "error"
      ...
      opt nil nil nil
      opt [] nil nil
      ;; CLJW: pred form is (or nil? sequential?) — CW regex-spec-impl uses literal form
      ;; UPSTREAM-DIFF: Different pred form
      opt :k ::s/invalid '[{:pred (or nil? sequential?), :val :k}]
      opt [:k] :k nil
      ...
      ;; CLJW: andre :k pred form matches CW regex-spec-impl literal
      andre nil nil nil
      andre [] nil nil
      andre :k :clojure.spec.alpha/invalid '[{:pred (or nil? sequential?), :val :k}]
...
(deftest describing-evaled-specs
  ...
  ;; CLJW: fn-sym returns nil, so plain predicates get ::unknown as form
  ;; UPSTREAM-DIFF: describe/form return ::s/unknown instead of 'odd? / 'clojure.core/odd?
