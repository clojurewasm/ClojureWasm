;; Upstream: clojure/test/clojure/test_clojure/reducers.clj
;; Upstream lines: 96
;; CLJW markers: 5
...
(ns clojure.test-clojure.reducers
  ...
  ;; CLJW: removed clojure.test.generative and clojure.data.generators (not available)
  (:use clojure.test))
...
;; CLJW: replaced defspec (clojure.test.generative) with equivalent deftest
(deftest test-reduced-always-returns
  (doseq [probe (range 0 20)
          to-end (range 0 5)]
    (let [len (+ probe to-end 1)
          nums (range len)
          m (zipmap nums nums)
          result (reduce-kv (fn [_ k v] (when (== probe k) (reduced :foo))) nil m)]
      (is (= :foo result)))))

;; CLJW: IndexOutOfBoundsException -> Exception (CW doesn't have Java exception hierarchy)
