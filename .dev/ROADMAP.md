...
| 5.3  | mark-sweep GC implementation per ADR-0028 + F-006. Landed in micro-commits per the split-into-tractable-units pattern that worked at 5.2: **5.3.a** (a25b1b9) empty skeletons for `runtime/gc/{gc_heap, mark_sweep, free_pool, root_set}.zig` per ADR-0028 + structure_plan.md decree. **5.3.b.1** (0ed5d2c) `GcHeap.alloc` body + AllocRecord side-table (live-list shape decision per docstring: side-table over intrusive next-pointer; preserves ADR-0009 8-byte HeapHeader invariant). **5.3.b.2** (839f859) `mark()` body + cycle invariant + `clearMarks` helper. **5.3.c.1** (437cd53) `sweep()` body with direct `rawFree` (no pool reuse yet). **5.3.c.2** (7567e31) free-pool push/pop fast-path + min-16-byte alloc per ADR-0028 §3 (FreeNode overlaid at offset 8; HeapHeader preserved at offset 0; `FreePoolKey.alignment` upgraded `u8` → `std.mem.Alignment` for rawAlloc/rawFree type-compat). **5.3.b.4** (1e1eaf4) comptime check `assertHeaderAtOffsetZero(T)` in alloc. **Remaining**: 5.3.b.3 (10 root walkers wired into Runtime/Env/Var/Fn/LazySeq/etc.; survey at `private/notes/phase5-5.3.b.3-survey.md`), 5.3.b.5 (`collect()` orchestration + adaptive threshold), 5.3.d (Phase 1-4 alloc-site migration `gpa.create` → `gc.alloc` + `Flags.marked` removal + `gc_*_not_supported` Codes removal per ADR-0017 a1) | [ ]    |
