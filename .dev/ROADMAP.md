...
| 5.2  | `runtime/value.zig` split per D-029 + ADR-0027 §5 + `.dev/structure_plan.md` F-004 decree. Decompose 590-line file (earlier row text incorrectly quoted 1335 — that is `analyzer.zig`) into `runtime/value/{value, nan_box, heap_tag, heap_header}.zig`. HeapTag + Value.Tag expand 32 → 64 per F-004 Group A/B/C/D table (A3 = nil_, D0 = big_int (rotation from slot 29 per ADR-0027 §5), D14/D15 = boolean_true/_false). New entries land as type-declared-only with `Code.feature_not_supported` stubs per `no_op_stub_forbidden.md`. `heapTagToTag` collapses to `@enumFromInt(@intFromEnum(.))` per simplify finding #8. `tag_ops.zig` empty skeleton lands (5.3 owner fills + picks parallel-arrays vs `TagOps` struct shape per ADR-0028 §4). 17+ caller imports migrate to the new paths          | [ ]    |
