...
| 6.6  | `runtime/regex/{compile,match}.zig` + `runtime/java/util/regex/Pattern.zig` + `lang/primitive/regex.zig` (`re-pattern` / `re-find` / `re-matches`). ADR-0031 Accepted (Alt 2 two-tier IR + lazy DFA over Pike-NFA). Cycle 1 in progress: recursive-descent parser handles literal / concat / dot / alternation `\|` / greedy quantifiers `*`/`+`/`?`; Pike VM thread-list driver with epsilon-closure dedup replaces straight-line tryMatchAt. ADR-0031 acceptance tests green: `(re-find #"a\|b" "xby")` → `{1,2}` and `(re-find #"a*" "aaa")` → `{0,3}`. Remaining for cycle 1: char classes `[...]` / escapes `\d \w \s` / anchors `^` `$` / `re-pattern` Value wiring through reader + clojure peer | [~] (`0288442`)        |
