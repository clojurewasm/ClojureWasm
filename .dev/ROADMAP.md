...
| 6.6  | `runtime/regex/{compile,match}.zig` + `runtime/java/util/regex/Pattern.zig` + `lang/primitive/regex.zig` (`re-pattern` / `re-find` / `re-matches`). ADR-0031 Accepted (Alt 2 two-tier IR + lazy DFA over Pike-NFA). Cycle 1 in progress: parser + IR for literal / concat / dot; tryMatchAt straight-line VM. Single-char + multi-char literal + `.` wildcard green; remaining for cycle 1: alternation / quantifiers / char classes / escapes / `re-pattern` Value wiring through reader + clojure peer                                                                    | [~] (`0288442`)        |
