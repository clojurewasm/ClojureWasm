...
| 6.9  | `lang/clj/clojure/string.clj` (Tier A, ~21 vars). Uses Phase 6.6 regex foundation. **Cycles 1-3 complete**: cycle 1 = ADR-0032 multi-file bootstrap loader + `(in-ns 'foo.bar)` + 3 vars. Cycle 2 = trim + predicate families (7 vars). Cycle 3 = indexing + replace string-only + escape (fn cmap) + reverse (6 vars). Cycle 4 lands `join` / `capitalize` / `split` / `split-lines` (via cw-native `re-find-from` intermediate primitive) per `private/notes/phase6-6.9-survey.md` §6 cycle 4                                                                                                                                                                                                                                                                                                                                                                                                       | [~] (cycles 1-3 done)  |
