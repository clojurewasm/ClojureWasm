...
- **Phase**: Phase 4 IN-PROGRESS. §9.6 cluster A done
  ...
  Cleanup wave in progress: 4.13 / 4.14 / 4.15 / 4.16 / 4.17 done.
  Remaining §9.6 rows (4.18–4.26.f) — protocol / object_header /
  host extension / deftype-raise / binding_stack / big_int /
...
## Active task — §9.6 / 4.18

`src/runtime/protocol.zig` dispatch table skeleton (per
ADR-0008) — `ProtocolDescriptor` + `MethodEntry` struct
declarations. No `dispatch` function yet (Phase 7 wires the
`CallSite` cache).

**Retrievable identifiers**:

- ROADMAP §9.6 task 4.18, ADR-0008 (protocol dispatch unify).
- `src/runtime/type_descriptor.zig` (4.17) already carries
  `MethodEntry { protocol_name, method_name, fn_ptr }` — the
  protocol-side descriptor lives in its own file with the
  protocol-name interned symbol and the list of methods.
