...
| F115 | Namespace naming strategy (JVM portability) | Design phase: clojure.* (JVM compatible) vs cljw.* (ClojureWasm-specific) naming convention. clojure.math/string/set are fine (real JVM namespaces). IO/system interop needs design: clojure.java.io compat layer, System/ interop routing, cljw.* for unique features (wasm FFI etc). Babashka model: clojure.* for compat, babashka.* for extensions. Goal: user code written with clojure.* should run on both JVM and ClojureWasm. |
