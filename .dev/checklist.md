...
| ~~F117~~ | ~~Cross-platform build (Phase 35X)~~     | Done: 4-heap-tag NaN boxing (D85), Linux x86_64+aarch64 Docker verified, macOS x86_64 Rosetta verified, CI workflow added. |
| F118 | Wasm SIMD (v128) + FFI deep (Phase 36)       | ~100 SIMD opcodes (Zig `@Vector`), multi-module linking, ClojureWasm-native API. Enables near-native C/Rust .wasm execution. Baseline: interpreter 14x-289x vs native (`bench/simd/results.md`). See `.dev/plan/phase35-custom-wasm.md` Phase 36 section. |
...
| F120 | Native SIMD optimization (ClojureWasm internals) | Investigate Zig `@Vector` for CW hot paths: string hashing, collection bulk copy (PersistentVector node ops), GC bitmap marking, `clojure.string` byte-parallel ops. Profile first to identify actual bottlenecks. Separate from Wasm VM SIMD (F118). Could share design with `cljw.simd` API if Phase 36 adds a Clojure-level SIMD namespace. |
