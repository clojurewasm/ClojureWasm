...
ある** — これが本リポジトリで `eval` を `anyerror!Value` にしている
コンパイルレベルの強制力。
...
1. なぜ `eval` の戻り値が `anyerror!Value` なのか。1 行で。
2. `Function._pad: [6]u8` は何のためにあるのか。1 行で。
3. `Builtin` を heap object ではなく 48-bit fn pointer にする利点は
   何か。1 行で。
