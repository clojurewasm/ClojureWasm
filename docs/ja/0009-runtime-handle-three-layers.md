...
Phase 2 の幕を開ける commit `91feef0` は、たった 1 つで 3 つのファイルを
同時に新設します。`runtime/runtime.zig` (150 行)、`runtime/dispatch.zig`
(202 行)、`runtime/env.zig` (skeleton 80 行)。**バラして commit したい
のに、できない**。なぜなら 3 ファイルが互いに型参照していて、3 つ
そろって初めてコンパイルが通るから。

この章は、その「3 ファイル同時投入」の構造的な必然性と、その背後にある
**3 層 (Runtime / Env / threadlocal) の責任分割**、そして v1 が
学んだ `pub var` ベースの dispatch を **struct field 化した理由** を
追体験します。本リポの理念の核です。
...
これは **循環参照**ですが、Zig の型システムは「同一ファイル内の型相互
参照」と同じく、**ファイル間でも型の前方宣言**を許します。ただし
「すべての参照先ファイルが存在する」が前提。ファイルが 1 つでも欠け
ていれば全体がコンパイルできません。
...
**だから 3 ファイル同時 commit が唯一の道**。本リポの「TDD red →
green → refactor」の基本サイクルでは、green になる粒度で commit
するのがルールなので、3 ファイルが green になる最小単位は 3 ファイ
ル同時投入です。

### Env は skeleton で投入する

しかし「3 ファイル丸ごと完全実装」を 1 commit に押し込むと diff が
読めなくなります。そこで **Env は最小限 (skeleton)** で投入：
...
`Namespace` / `Var` / `BindingFrame` は **Phase 2.3 (commit
e20acaa)** で埋めます。第 0011 章で扱います。
...
**Q1**: 失敗。A は C.C を参照するが C.zig が存在しない。

**Q2**: 失敗。B は C を参照、A も C を参照。やはり C.zig 不在で死ぬ。

**Q3**: 成功。3 ファイルが同時にコンパイル可能になり、相互参照が
解決する。

これがまさに `runtime.zig` / `dispatch.zig` / `env.zig` で起きて
いる現象。Zig の型解決は「ファイル単位で全部見えていれば循環でも
OK」という性質を持つので、**3 ファイルを 1 commit にまとめる**
のが正解。
...
│   vtable: ?VTable                                             │
│   heap_objects: ArrayListUnmanaged(HeapEntry)                 │
└──────────────────────────────────────────────────────────────┘
...
**`heap_objects` は Phase 5 mark-sweep GC が来るまでの "繋ぎ"**。
Layer 1+ がヒープにオブジェクトを作ったとき、`(ptr, free_fn)` を
ここに登録しておくと `Runtime.deinit` が一括で free します。
Layer 0 (= `runtime.zig`) は具体的な型 (例えば `tree_walk.Function`)
を **知らない** まま、関数ポインタ経由で free できます。
...
「Env が Runtime を指す」一方向で、Runtime は Env を知りません。
**この向き** が大事：複数 Env が 1 つの Runtime を共有できるのは、
Runtime が Env リストを持たないから。**v1 の nREPL race condition の
直接の修正点** がここ。詳細は第 0011 章。
...
「threadlocal は悪、撤廃すべき」ではなく **「正しい threadlocal が
ある」**。Clojure dynamic var を threadlocal なしで再現すると性能・
意味論の両面で破綻します。
...
これは **v1 (89K LOC) の retrospective**。v1 の `dispatch.zig` は
こんな形でした：
...
これは **モジュールレベルの可変グローバル**。起動時に `tree_walk` が
`dispatch.callFn = &tree_walk.callFnImpl;` のように上書きします。
...
**`?VTable`** で nullable にしている理由は **起動の段階性**。Phase 2.6
で TreeWalk が `installVTable(rt)` を呼ぶまで `vtable == null`。これは
構造的な保証 (= 「null の時 callFn が走らない」) でもあります。
...
**コンパイル時の依存方向は Layer 0 から Layer 1 への呼び出しを禁止し
たまま、実行時の関数呼び出しは Layer 0 → Layer 1 が成立**。これが
依存反転 (Dependency Inversion) です。
...
- 2.1 時点では Layer 1 (`eval/`) の TreeWalk もアナライザもまだ存在
  しない。実装が無いものを注入する関数を書く意味がない。
- 「`vtable == null` のままでも runtime は構築できる」という抽象
  境界を明示する。Phase 2.6 で TreeWalk が完成したとき、初めて
  `tree_walk.installVTable(rt)` が登場する。
- これは **YAGNI** ではなく **段階性**。Phase 2.1 の commit に
  `installVTable` を書いたら、それは「呼ぶ場所のないコード」になる。
...
`std.Io.Mutex.lock` が `io` を引数に取るので、**lock を取るためには
io が手元にないといけない**。`io` は普通 main から流れてくる。
グローバル変数にしておくのは設計のサボリ。
...
- `heap_objects` は `std.ArrayListUnmanaged(HeapEntry) = .empty`
...
- **3 層を Day 1 から固定**: v1 は 18 ヶ月かけて 3 層に到達したが、
  本リポは Phase 2.1 で型を確定。Field 追加だけで Phase 5/14/15 を
  迎える設計。
- **`pub var` を最初から却下**: v1 retrospective + Zig 0.16 idiom +
  multi-tenant 要件の三方一致。strategic note `private/2026-04-24
  _runtime_design/REPORT.md` で 5 案比較した結果。
- **3 ファイル同時投入を恥じない**: 循環 import を避けるための
  分割は人工的な肥大化を生む。**自然な責任境界が 3 ファイルに
  またがる場合、3 ファイル commit が正解**。
...
2. なぜ `runtime.zig` / `dispatch.zig` / `env.zig` を 1 commit に
   まとめるのか？ 1 行で。
3. なぜ `pub var callFn` ではなく `Runtime.vtable: ?VTable` field
   なのか？ 1 行で。
...
第 0010 章: [KeywordInterner を rt-aware に昇格](./0010-keyword-rt-aware.md)

— Phase 1 では module-level mutex 風で済ませた keyword interning を、
Runtime ハンドル導入直後に **rt-aware (mutex via `rt.io`)** へ refactor
します。なぜ Phase 2.1 と 2.2 を別 commit にしたか、cell layout を
固定した理由、`internUnlocked` という名前選定の意図を見ていきます。
