...
`(+ 1 2)` が `3` を返すには、ホスト言語（Zig）の **実体ある関数**
が `+` というシンボルから引ける必要がある。本章では **math
primitives**（`+ - * = < > <= >=` と `compare`）、**core predicates**
（`nil? true? false? identical?`）、そして 1 行の `registerAll(env)`
で `rt/` 名前空間に詰め込み `user/` に refer する **起動コード** を
扱う。

3 commit を 1 章に統合するのは、これらが **「素の builtin を生
やす」** という同一の概念を 3 段階で完成させるから。math だけ /
core だけ / registerAll なし — どれが欠けても動かない。
...
方で実装依存だが、本リポは **NaN-boxed i48 そのものが値**なので
**必ず同じ u64**。よって常に true。
...
- v1 は primitive を heap に持つので **GC roots に登録**が必要。本リポは **48-bit fn ptr** で GC を経由しない。
- v1 の `core_lib_<group>` 接頭辞を、**`primitive/<topic>.zig`** に直してレイヤ語を明示。
- Clojure JVM は **`clojure.core` 自身が `.clj` ファイル**で、JVM intrinsic だけ Java。本リポは **Phase 3 で `core.clj` Stage 1** を入れる（macro layer）。それまで **すべて Zig**。
...
1. **`(+)` が `0` で `(*)` が `1` なのに、`(-)` がエラーなのはなぜ？** 1 行で。
2. **`(true? 1)` と `(if 1 :a :b)` の結果はどう違う？** 1 行で。
3. **`registerAll` を 2 回呼んでも環境が壊れないのはなぜ？** 1 行で。
...
— Phase 2 の **完成**。`main.zig` を Read-Print から **Read-
Analyse-Eval-Print** に昇格させ、`test/e2e/phase2_exit.sh` で 3 つの
exit criterion を CLI 経由で pin する。`(let* [x 1] (+ x 2))` が
`3` を返す瞬間を見届ける。
