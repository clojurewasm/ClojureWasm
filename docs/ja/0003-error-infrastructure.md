...
から有効にする。**エラー位置 (`<file>:<line>:<col>`)** と **構造化
された `Info` ペイロード**を全エラーに付け、Reader でも Analyzer でも
TreeWalk でも、エラーの読み心地を一貫させるための土台がここで決まる。

Zig の `error{ ... }!T` には文字列メッセージを乗せられない、という
制約をどう回避するかが設計の主役。**threadlocal な `last_error`**
を 1 個持ち、`Error` タグの返却と同時に Info を書き込む方式を採る。
...
ランタイム（v1）。そこでは **error.zig が Phase 後半まで存在せず**、
`error.OutOfMemory` のような Zig 標準エラーが裸で投げられていた。
結果、Phase 30+ で位置情報を後付けしようとしたとき、**500+ の
エラー site すべて** を書き直す羽目になった。

ClojureWasm v2（本リポ）は ROADMAP 原則 **P6** を Day 1 から
立てることで、この罠を踏まない。
...
ありえない値 → センチネルとして使える。`Optional(u32)` を選ばずに
**ゼロ値で sentinel** にしているのは、`SourceLocation{}` という
リテラルを `null` チェックなしで使いまわしたいから。

**Q3**: 両方 `0`（つまり「ファイルだけ判っている、位置は未知」）。
これは Reader が EOF で出すエラーや、CLI の `-e` 引数で `file = "<eval>"`
だけ判っているケースで自然に使える。

**Q4**: 1 行: 1 行が 65535 列を超えることは現実上ない（`u16` で十分、
`u32` は無駄）。バイト切り詰めの最適化。
...
されるが、payload は `T` 用に予約されているので **ユーザ側が好きな
情報を乗せる場所はない**。

### 解決: threadlocal な「最後のエラー」

C 系言語の `errno` の発想を借りる。エラー時に **構造化 Info を
threadlocal バッファに書き込んで** から、Zig の `Error` タグだけを
返す。`catch |err|` 側は `getLastError()` で詳細を取り出す。
...
- `threadlocal var` — Zig の組み込み機能。各スレッドが独立した値を持つ。
- `[512]u8` の固定バッファに `bufPrint` で書き込む。**動的 alloc しない**
  ので OOM 経路でも動く。
- `?Info` (optional) — 未取得時の null と区別できる。

### Clojure 動的 var との整合

Clojure では `*out*` / `*err*` / `*ns*` 等の **動的 var** は
**スレッドごとにバインディング**を持つ（per-thread `Var` rebind）。
本リポの threadlocal `last_error` も同じ意味論でスレッドに閉じる。
**Phase 15 の concurrency 拡張で問題にならない**ことが、この時点で
保証される。
...
つまり「`last_error` を threadlocal にする」は、Clojure の意味論との
**意図的な一致**であって、 偶然の最適化ではない。
...
1. **メッセージを書く**: `bufPrint` で 512 byte buffer に書く。
   オーバーフロー時は末尾を `"..."` に潰すフォールバック。
2. **Info を保存**: kind / phase / message / location を構造体で書く。
3. **Error タグを返す**: `kindToError(kind)` で Zig 側の error タグへ
   1:1 で写す。`return setErrorFmt(...)` という 1 行で全部済む。
...
- `catch blk: { ... break :blk ... }` は Zig のラベル付きブロック
  expression。エラー回復値を式として返せる。
- `@memcpy(msg_buf[509..512], "...")` で末尾 3 byte を上書き。
  buffer 全長 512 を返している（truncate 表現として）。
- `return` 1 行で副作用 + 戻り値 — 呼び出し側は
  `return setErrorFmt(.eval, .type_error, loc, "f: bad arg", .{})` の
  ようにスッキリ書ける。
...
`loc` を **第 2 引数**に置く決断が、章末の演習 3.3 まで続く伏線になる。
...
呼び出し側 (`(+ ...)` という Form) の位置がエラーに付く。`loc` を
引数にしておかないと、Phase 後半で **全 builtin のシグネチャを
書き直す**羽目になる（v1 の Phase 30 で実際に起きた）。

### `expect*` ヘルパ

builtin 1 個 1 個に「型チェック → エラー返却」を手書きさせるとブレる。
`expect*` で集約する:
...
**`(rt, env, args, loc)`** に signature を拡張するときに、`loc` の
枠をすでに掘ってあるから。`error.zig` 冒頭のコメント:
...
すなわち **「いつか動かす」という宣言を doc-comment に書いておく**
のがポイント。「P6 を Day 1 から」と「P2 (final shape on day 1)」
の合わせ技。
...
- v1 は「位置情報は後段で集めればよい」と判断 → Phase 30 で 500+ 箇所
  書き直し。本リポは **`BuiltinFn` 引数に `loc` を Day 1 から組み込む**。
- Clojure JVM の `Throwable` 階層は **Class explosion** の温床。本リポは
  `Kind` enum 12 種で **flat** に管理（ROADMAP §A6 ≤ 1000 LOC を支援）。
...
1. なぜ Zig の `Error!T` だけではエラー詳細が運べないのか？ 1 行で。
...
— Phase 1 の `ArenaGc` は「個別 free しない、`deinit` で全部解放」
という single allocator。さらに Day 1 から **`suppress_count`**
（マクロ展開中の collection 阻止）と **`gc_stress`** flag（Phase 5
mark-sweep のテスト用）を仕込んでおく仕組みを掘り下げます。
