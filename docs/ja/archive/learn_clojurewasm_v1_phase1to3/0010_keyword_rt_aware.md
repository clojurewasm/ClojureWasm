...
> 注: 以下のコード片に登場する `std.array_hash_map.String(*Keyword)`
> は、当時のコミット `b60924b` / `07d5c34` 時点では旧名
> `std.StringArrayHashMapUnmanaged(*Keyword)` で書かれていました。
> Zig 0.16 で前者が deprecated 別名になったため、本書では現在の
> 正規名 `array_hash_map.String` で統一表示しています（型としては
> 同一の string-keyed array hash map です）。
...
Phase 2 ではその上に API を被せただけ** ということです。具体的に
何が動いて何が動かなかったかを並べると、変更の輪郭がはっきり
します：

| 項目                                                | 変化 | 補足                                                                  |
|-----------------------------------------------------|------|-----------------------------------------------------------------------|
| `Keyword` struct のフィールド順                     | ×   | Phase 1 で凍結済み                                                    |
| Murmur3 hash 関数 (`computeHash`)                   | ×   | 純関数なので無関係                                                    |
| `intern` API シグネチャ                             | ○   | `(self, ns, name)` → `(rt, ns, name)`                                |
| `find` が返す `Value` の bit 表現                   | ×   | `Value.encodeHeapPtr(.keyword, kw)` で同じ                            |
| 内部 `std.array_hash_map.String(*Keyword)` の使い方 | ×   | 中身は `internUnlocked` に同じコードが残った                          |
| test 組み立て                                       | ○   | rt-aware test では `TestFixture` (`std.Io.Threaded` + Runtime) が必要 |
...
top-level の rt-aware ラッパは、シンプルに 3 行で書けます：
...
- 結局のところ、この章は **「メモリレイアウトは Phase 1 で凍結し、
  API 表面だけを Phase 2.2 で rt-aware に被せ替えた」** という
  workaround を残さない作法の話です。
- mutex は Phase 2 ではコスト計測誤差ですが、Phase 15 で
  `keyword.zig` を再度触らずに済ませるための **未来への伏線** です。
- `internUnlocked` という suffix が「lock 状態を関数名で表す」Zig
  の文化を体現しており、bootstrap・テスト・rt-aware ラッパの 3 用途
  を 1 ファイルに同居させています。
...
第 0011 章: [Env を完成版に — Namespace, Var, threadlocal binding frames](./0011_env_namespace_var.md)
