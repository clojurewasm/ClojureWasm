...
落とす** Phase-2 のバックエンドを扱います。Clojure ランタイムが
**初めて式を実行する瞬間** の章です。10 種類の Node variant を switch
で dispatch し、`Function` を heap に allocate し、48-bit ポインタ
表現の `builtin_fn` を呼びます。そして `installVTable` で Layer-0 →
Layer-1+ の依存反転を完成させます。

Phase 4 で bytecode VM が登場しても、この TreeWalk は **消さずに残し
続けます**。**dual-backend 検証**（ROADMAP §4.4）の参照実装として、
VM の出力を TreeWalk と比較するためです。
...
経由の error union 規則** のためです。`runtime/dispatch.zig` の
`CallFn = *const fn(...) anyerror!Value` を呼んだ瞬間 `evalCall` の
error set が anyerror に広がります。Zig の error union は **狭めるのは
...
signature を `anyerror!Value` にしておけば error set 追加だけで済みます。

なお `eval` 全体を `!Value` (inferred error set) にして `evalCall` 経路
で再帰させると、Zig は `error: function with inferred error set called
recursively / cannot resolve inferred error set` でコンパイル失敗します。
**再帰関数 + inferred error set を解決できない** ため、明示的に
`anyerror` を書く必要があります。これが本リポジトリで `eval` を
`anyerror!Value` にしているコンパイルレベルの強制力です。
...
ホットパスは **配列 index と pointer load** のみです。HashMap も string
compare もありません。これが第 12 章「解析を前倒しする価値」の正味です。
`var_ptr.deref()` は Phase 3 で dynamic binding stack を実装したら
override も拾うようになります (Phase 2 では root を返すだけ)。
...
たとえば `(let* [x 1 y 2] x)` を eval すると、`locals` 配列は
`[nil, nil, ...]` で初期化されたあと、x bind で `[1, nil, ...]`、
y bind で `[1, 2, nil, ...]` と進み、body の `x` が `locals[0]` を
読んで 1 を返します。slot 2 以降は触らず、call が終われば locals
配列ごと stack から消えます。
...
ためです。`encodeHeapPtr` は `&function` のアドレスが 8 で割り切れる
ことを要求します。`HeapHeader` は 2 bytes、その後 6 bytes の padding
を挟むことで `Function` 全体の `@alignOf` が 8 になります。テスト:
...
実は `_pad` が無くても `Function` の自然 alignment は内部最大
(`*const Node` = 8 bytes) で決まるので、結果として 8 byte alignment
は保たれることが多いです。だが Zig の field reorder/padding に
依存します。`_pad` 明示は **将来 field 入れ替えても alignment が
壊れない保険** で、`@alignOf(Function) == 8` が変わったら debug
build で alignment テストが落ちる ⇒ release で
`assert(addr & 0x7 == 0)` が hit する前に検出できます。
...
します。Phase 5 で GC が走り出したら Function も普通の heap object
として trace 対象になります。

### Phase 2 の Function は **closure capture を持たない**

`Function` 構造体に `captured: []Value` フィールドがありません。

```clojure
(let* [x 1] (fn* [y] (+ x y)))   ; ← Phase 2 では動かない
```

body の `x` は `LocalRef{ slot: 0 }` を指しますが、生成された
Function が呼ばれる時点では **新しい locals 配列** が用意されていて
外側の `x` がいません。Phase 3+ で `Function.captured: []Value` を
追加し、analyse 時点で free variable を集めて allocFunction 時に
固める予定です。
...
dispatch** します。これが `installVTable` で `treeWalkCall` 自身に
解決されるので結局自身を間接呼出。なぜ vtable 経由か：**Phase 4 で
VM が来たら `vtable.callFn = vmCall` に差し替えるだけで eval 全体が
VM に切り替わる** からです。frontend は backend に依存しないという
レイヤ規律を vtable 一段で実現しています。
...
ArityMismatch を投げる典型ケース: `(fn* [x] x)` を 0 引数や 2 引数で
呼ぶとどちらも Mismatch (arity=1 で has_rest=false なので正確に 1
必要)、`(fn* [x & rest] x)` を 0 引数で呼ぶと min 1 を満たさず
Mismatch、ただし 1 引数以上なら OK です。
...
です。heap object **無し** で fn pointer 自体を Value に詰めます。
...
ポインタは 8-byte align で 45 bit に圧縮しますが、**関数ポインタは
align できない** ので 48 bit そのまま使います (`builtin_fn` だけ別
tag `0xFFFF`)。`comptime FnPtr` で型を呼び出し側に決めさせるのが
key — `value.zig` 自身は `dispatch.BuiltinFn` を import しなくて
済みます (zone 違反回避)。Roundtrip としては
`Value.initBuiltinFn(&f).asBuiltinFn(@TypeOf(&f))` が `&f` と一致
します。呼び出し側 `callBuiltin`:
...
ROADMAP の **「sub-10 ms cold start」** (原則 P10) に直接効く設計です。
...
3 つの fn pointer を `Runtime.vtable` に書き込みます。`Runtime.vtable:
?VTable` は `Runtime.init()` 直後は `null`、誰かが install するまで
`vt.callFn` は呼べません (`evalCall` が `if (rt.vtable) |vt|` で守る)。
...
する (vtable null) ためです。`valueTypeKey` (multimethod 用、Phase
6+) と `expandMacroStub` (Phase 3 で wires) も stub で入れておきます
— **vtable の全フィールドを埋めないと struct literal が通らない**
ためです。

---

## 8. 10 variant の dispatch まとめ

`call_node` 以外の 9 variant は素直な eval、`call_node` だけが
**vtable 経由で別関数を呼ぶ** 特殊扱いという構造です。これにより
Phase 4 の VM がここを差し替えるだけで全体が切り替わります。
...
| `.call_node`  | callee/args eval → `vtable.callFn` で dispatch        |

---

## 9. 将来設計: `recur` の threadlocal pattern

Phase 2 にはまだ `loop*` / `recur` がありません。だが eval の戻り値型
を `anyerror!Value` にしたのは Phase 3+ でこの pattern が来るのを
想定しているからです。Zig には本来の TCO がないので、recur は
**threadlocal で引数を stash + tagged error を投げる + loop の
catch で受ける** pattern で実装します (v1_ref で実証済):
...
threadlocal を使う理由: error union で値を運べない → 別 channel が
必要だからです。同一スレッドで複数の `loop*` が並行することは無い
(recur は **直近の loop** にしか飛ばない) ので thread safety は OK。
Phase 2 の eval signature を `anyerror!Value` にしておけば Phase 3
で `EvalError.RecurSignaled` を error set に加えるだけで動きます。
`pending_recur = null` を忘れると次の recur が混ざる点だけ要注意です。
...
Phase-2 exit criterion **両方** がここで通ります:

- `(let* [x 1] (+ x 2))` → 3
- `((fn* [x] (+ x 1)) 41)` → 42

ただしこの commit 単独では builtin `+` が未 register。test 内で
**inline の `builtinPlus`** を `Env.intern` して動かしています (zone
違反回避のため)。次章 0015 で `lang/primitive` から正式 register
され、CLI からも `cljw -e "(+ 1 2)"` が動くようになります。
...
しています（cold start を意識した P10）。vtable は `pub var` global
ではなく **`Runtime.vtable: ?VTable`**（§13 の禁則回避、並列テストにも
対応可）です。recur の threadlocal + error pattern は v1_ref で実証
済みで、本リポジトリでも Phase 3 で同じ手筋で導入する予定です。

---

## この章で学んだこと

- 結局のところこの章は、**`eval` が 10 variant の switch + `vtable.callFn`
  経由 1 段間接で完結する** 構造の披露である。`anyerror!Value` という
  signature が、vtable の error widen と Phase 3 recur の前借りを
  同時に成立させている。
- Builtin を **48-bit fn pointer** に詰めて heap を経由しないこと、
  vtable を `?VTable` にして `Runtime.init` 直後に install すること
  の 2 点が、cold start (P10) と layer 反転を同時に支える。

---

## 次へ

第 15 章: [Primitive 群と `registerAll`](./0015_primitives_and_register.md)

— `+` `-` `*` `=` `<` `>` `nil?` `true?` `identical?` などの組込み
関数群を `lang/primitive/{math,core}.zig` に置き、`registerAll(env)`
で `rt/` 名前空間に intern し、`(refer 'rt)` 経由で `user/` から参照
できるようにします。**ついに `cljw -e "(+ 1 2)"` が CLI から動く**
章です。
