...
(Error quality is non-negotiable) の **暫定形** で、`@errorName(err)`
だけ表示します。Phase 3 task 3.1 で `<file>:<line>:<col>` 込みの本格
P6 報告に置き換える予定です（章末で予告）。

### 起動順の依存方向

`Runtime.init → Env.init → installVTable → registerAll` の順序には
明確な依存関係があります。`Env` は `*Runtime` を `.rt` フィールドに
保持し、`Namespace` の allocator や `KeywordInterner` を runtime
経由で取ります。**Runtime が parent、Env がその下のレイヤ** という
親子関係が、`Env.init(&rt)` のシグネチャに現れています。

`installVTable` を `registerAll` の後に置くと、`tree_walk.eval` の
呼び出しが vtable を介さないため、`Function` heap 値（`fn*` で作る
closure）の呼び出しが panic します。`((fn* [x] (+ x 1)) 41)` の
テストが落ちる、という形で破綻が即座に観測できます。

---

## 2. printValue — Phase 2 surface のみ

`printValue` は `main.zig` の **暫定実装** で、Phase 2 が扱える値の
範囲だけを正しく印字します。
...
### なぜ heap kind は `#<list>` placeholder か

Phase 2 では heap collection（list / vector / map）は **まだ存在
しない** です。primitive を呼ぶときの `args` slice は analyzer が
直接 `[]Value` を作るため heap を経由しません。よって `printValue`
が heap 型に出会うのは **builtin_fn を返す primitive を直接 print**
するレアケースのみとなります。

将来 Phase 3 で list / vector が入ったら、`else => |t| #<{s}>` の
枝が当たり始めます。そこで **`runtime/print.zig` に切り出して本格
`pr-str` を実装** するのが §9.5 task 3.8 の予定です。

### `##NaN`, `##Inf`, `##-Inf` の特別処理

Clojure (1.9+) は IEEE-754 の特殊値を **読める形** で印字します。
...
出すので、Clojure 互換に明示的に分岐しています。これは小さな細部ですが、
**「pr-str した結果が再度 read できる」** という Clojure の不変
条件を Phase 2 から守るためです。
...
ので、per-eval で arena を `.deinit()` して掃除する意義が薄いです。
**プロセス終了でまとめて捨てる** 方が単純で速い、という判断です。

これは ROADMAP **P5 (do not optimise prematurely)** の典型応用です。
速度・容量の最適化は **動くものができてから測定して** やる、という
原則そのもので、Phase 2 の cljw は対話 REPL ではないので、メモリの
長期 fragmentation を心配する必要がありません。

将来 REPL（Phase 3+）が来たら、各 form ごとに per-eval arena を切る
かどうかは別途設計事項です。本章の時点では **「動くものを最短で」**
を優先します。

### error path の典型形

3 段の error path はすべて同じ shape をしています。`reader.read()`、
`analyzeForm(...)`、`tree_walk.eval(...)` のいずれも `catch |err|
{ stderr.print("...error: {s}\n", .{@errorName(err)}); stderr.flush();
std.process.exit(1); }` で終わります。Phase 3 task 3.2-3.4 で
`setErrorFmt` 経由の P6 統合に置き換えると、3 つとも単一の
`printError(stderr, rt.last_error)` に畳まれる予定です。

`locals` 配列は **stack 上** にあり、`tree_walk.MAX_LOCALS` が固定
なので heap 取得は不要です。`try stderr.flush()` を **exit 前に必ず**
呼ぶ点も重要で、buffered な stderr は flush 忘れで黙ってしまいます。
...
が満たされたかを **shell script で pin する** のが `7d9fe5f` です。

### exit criterion の 3 ケース

`test/e2e/phase2_exit.sh` の核心は次のとおりです。
...
`set -euo pipefail` で **どれか 1 つでも非ゼロ exit したら即終了**
し、`2>&1` で **stderr もキャプチャ** することで CLI が「Read
error: ...」を stderr に書いたケースも検出できます。`-e` で渡す式
には shell 特殊文字（`!`、`*foo*`、`` ` ``）を含めないこと
（`.claude/rules/cljw_invocation.md` 参照）。3 ケースとも安全です。

### なぜ executable spec か

代替案を 3 つ考えると本質が見えます。

**A: textual な exit criterion を ROADMAP に書くだけ** —
**ドリフト** が問題です。半年後に誰かが `+` を変えても、ROADMAP は
静的なテキストなので **検出されません**。CI も走りません。

**B: zig test で `expectEqualStrings`** — `main.zig` を経由しない
ことが問題です。**CLI 全体（arg parsing + RAEP + stdout flush）が
含まれません**。

**C: bash script で実バイナリを叩く ✓** 採用案です。**`zig build`
を毎回走らせ、`zig-out/bin/cljw -e '...'` の出力を文字列比較** して、
CLI まで含めた end-to-end の検証が 1 ファイルに収まります。

### `test/run_all.sh` への配線
...
の exit criterion が組み込まれました。**ROADMAP §11.6 (gate timeline)**
で Active gate に昇格しています。これが緑になった瞬間が **Phase 2
完了の正式な印** です。
...
- 結局のところこの章は **Phase 2 の全層を `main.zig` で wire し、
  3 つの shell ケースで CI に固定した**章です。
- `Runtime → Env → installVTable → registerAll` の起動順が依存
  方向そのもので、入れ替えると closure 呼び出しが panic します。
- exit criterion を `bash` で書くからこそ、CLI 全体（arg parse +
  RAEP + flush）込みの end-to-end が単一ファイルで pin されます。
...
始まります。教科書で言えば「error 報告は Day 1 から本気でやる」を
真に実現する章です。
