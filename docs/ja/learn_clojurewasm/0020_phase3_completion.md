...
分岐を 1 つ書き足すだけで済む形になっています。Zig 0.16 の `switch` は
tagged union に対して exhaustive を要求するため、enum メンバーとして
存在する `transient_vector` などの未対応タグを `else` で受けないと
compile error になります。`else` arm が安全な fallback として存在する
おかげで、新しい HeapTag を増設しても `print.zig` 側の追従は段階的に
済みます。
...
`TryNode.body` の組み立ては、body form の数で分岐させます。`(try 1
...)` のように body form が 1 個なら constant `1` をそのまま使い、
`(do 1)` でラップしません。0 個なら `nil` constant を返し、複数なら
`(do ...)` で束ねます。これは `if` / `let*` / `fn*` の helper と同じ
慣用です。
...
エラー化します。`(loop* [x 0] (recur 1 2))` のように binding 数と
recur 引数数が食い違う場合、Analyzer は `expected 1 arg, got 2` の
syntax_error を返します。tail-position チェック（`recur` が末尾位置に
あるかの判定）は `is_tail` フラグを `analyze` 全体に thread する大きな
変更になるため、評価器の形が見えてから 3.11 で codegen 寄りに入れる
ことにしました。
...
`ExInfo` が GC で回収されると、`message` が指していたバイト列の所有権が
消えるため、caller がその Value を保持していてもポインタ先は解放済みに
なります。代わりに `string_collection.alloc` で **新しい String Value**
を確保して返せば、Value の lifetime が ExInfo から独立し、GC trace の
観点でも独立した root を持つので安全になります。1 行のコストで、回収
順序のバグを永久に潰せる安全な作りです。
...
具体例として `(try (do (prn :body) (throw (ex-info "x" 0))) (catch
ExceptionInfo e (prn :caught) :handled) (finally (prn :finally)))` を
評価すると、stdout には `:body` → `:caught` → `:finally` の順で印字
され、最終戻り値は `:handled` になります。catch がマッチした場合でも
finally は走り、戻り値は catch の結果になる、という三状態の (b) が
動いていることがこれで確認できます。
...
`closure_bindings` を `rt.gpa` に乗せて allocate し、`freeFunction` で
解放するつくりにしているのは、Analyzer の arena が input loop が
一巡したら deinit されるのに対し、`Function` Value は env / heap の側で
生きつづけ、後の呼び出しで closure を参照するからです。lifetime が
異なるので別の allocator に乗せる必要があります。
...
評価器の入り口は次の二つの公開 API に集約します。`tree_walk.eval` が
Node tree を再帰的に walk し、`tree_walk.callFn` が closure を locals
に書き戻して呼び出します。
...
これで `(loop* [x 0] (if (< x 3) (recur (+ x 1)) x))` が `3` を返し、
...
`main.zig` の startup chain は次の順序で組み立てます。

1. `Runtime.init`
2. `Env.init`
3. `installVTable`
4. `primitive.registerAll`
5. `macro_transforms.registerInto`
6. `bootstrap.loadCore`
7. user input loop

`loadCore` を `registerAll` の前に置くと、`+` などが未登録の状態で
`core.clj` を評価することになります。Phase 3 段階の `core.clj` は
`def` / `fn*` / `if` の special form だけで構成されているので
たまたま起動には成功しますが、将来 `(def inc (fn* [x] (+ x 1)))` の
ような primitive 依存の式を追加した瞬間に「`+` が見つからない」と
未定義参照で落ちます。事故が表面化しないうちに「primitive → macro →
bootstrap」の順序を守るのが筋です。
...
- Phase 3 後半は「制御フロー（recur / throw を Zig error として運ぶ）」
  「ブートストラップ（最小 `core.clj` を `@embedFile` で焼く）」
  「修正方針（§17 を ADR 0002 で初運用）」の三本柱が同時に決着した章。
- closure を `slot_base` まで切り詰める判断と、`ex-message` が新規
  String を返す判断は、どちらも Phase 5+ GC を見据えた lifetime 設計。
- ROADMAP §9.5 / 3.14 の `{}` を `0` に直したのは bug fix ではなく
  §17 四ステップの amendment であり、change-bar ではなく ADR + git log
  に履歴を残す方針が実地で機能した最初の例。
