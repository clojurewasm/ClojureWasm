...
の `HeapTag` で 8 種類の heap object を区別します。
...
heap 型を 1 つ増やすたびに、必ず 4 つのファイルを揃って触ります。
...
これは **A2「新機能は新ファイル」** と **P3「core stays stable」** を
体現した「足し算だけで進む」経路です。3.5 と 3.6 はまさにこのレシピを
2 回連続で踏むタスクでした。

### HeapTag が 4-bit に収まらなければならない理由

`HeapTag` の **値域は 0–15 に収める** 必要があります。NaN-boxing の
payload は 64-bit のうち上位 13 bit が "quiet NaN signature"、下位 48
bit が pointer payload、その間の **4 bit** が tag 領域です。15 を
超える tag は別 bit に侵食して NaN-box が壊れます。

`enum(u8)` という記憶域型を選んでいる点も注意です。コードの取り回し
は 8-bit でも、Value への書き込み時 (`encodeHeapPtr`) に 4-bit field
へ閉じ込めます。`@intCast` で overflow すれば runtime panic — 4-bit
を超えた tag は **コンパイルでは気づけず実行時に死ぬ** ので、enum
定義時点で人間が守る必要があります。
...
allocator から取るべきか」でした。素朴には analyzer の per-eval arena
を使いたくなります。
...
これは `(def x (quote (1 2 3)))` で破綻します。`def` は Var に Value
を bind しますが、analyzer arena は **次の式の analyse が始まる前に
reset される** — Var が抱えた `*Cons` ポインタは即時 dangling pointer
になります。
...
で `Runtime.deinit` 時に解放するよう登録することです。
...
unit test では arena allocator を渡したい (テスト終了時 bulk free が
楽) ので、両方 export して用途で使い分けます。
...
これは Phase 5+ で mark-sweep GC が入るまでの暫定構造です。GC 移行時
には `trackHeap` の callback リストごと「GC root テーブル」に置き
換わります。
...
1. **`_pad: [6]u8`** — `HeapHeader` が 2 byte なので、続く `bytes`
   slice ポインタの 8-byte alignment を確保するための明示パディング
   です。`comptime assert(@alignOf(String) >= 8)` がコンパイル時に
   守ります。NaN-boxed pointer encoding は **下位 3 bit を 0 にする**
   ことを要求するので alignment が崩れると即時 panic します。
2. **`errdefer rt.gpa.free(owned)`** — `dupe` は成功したが `create` で
   OOM、というパスで bytes が orphan するのを防ぎます。Zig の
   `errdefer` は try が rebind するときだけ走るので、successful path
   では free されません。
3. **`alloc` の戻り値は `Value`** — String pointer をそのまま渡しません。
   `encodeHeapPtr(.string, s)` で NaN-box し、tag bits も焼きます。

print 側 (`main.zig::printString`) は対称な escape table を持ちます。
...
3.5 の暗黙の不変条件です。escape 表に `\u{xxxx}` を足したくなったら、
Reader 側にも対応エントリが要ります。
...
A6「≤ 1000 lines per file」をすべて満たします。
...
- Clojure JVM の `()` と `nil` の区別は EmptyList class hierarchy が
  支えています。本リポジトリは Value tag が単一（`HeapTag.cons`）で、
...
- 結局のところこの章は **「heap-backed Value の lifetime は
  per-eval arena ではなく `rt.gpa` + `trackHeap` でなければならない」**
  という 1 つの結論に収束します。`(def x ...)` で Var に bind した
  瞬間に arena reset で dangle するのが破綻シナリオです。
- heap 型を 1 つ増やすには `value.zig / collection/<name>.zig /
  analyzer.zig / printValue` の 4 ファイルを揃って触る、という決まった
  レシピがあります。
- HeapTag が 4-bit 制約に収まらないと NaN-box が崩れる。`enum(u8)`
  という型は記憶域に過ぎず、書き込み時に人間が枠を守る必要があります。
