<!-- Per-concept chapter. docs/ja/0020-phase3-completion.md -->
...
上向きの import が必要になり zone 違反です。`runtime/print.zig` に
移して Layer 0 に閉じさせれば、どの層からでも素直に呼べます。
...
足したとき（3.10 で `ex_info` を入れたとき）に、`else` arm の前に分岐
を 1 つ書き足すだけで済む形になっています。

### 1.2 移動の最小性を守る

3.8 は機械的なリファクタリングです。「移動ついでに書き直したい」誘惑
は強いものの、ここで shape を変えるとレビューが破綻します。本リポ
ジトリは関数名と引数名を変えず、`pub` する位置だけ動かす最小手術に
...
Q2: なぜ `else` arm が exhaustive 必須なのか。
...
本リポジトリは Clojure JVM の `TryExpr.catchExprs: PersistentVector` に
寄せて **flat な配列** で持ちます。Phase 3 では `class_name` が
`"ExceptionInfo"` 1 種類だけですが、Phase 5+ で multi-catch / Class
解決が来たときに線形 walk のロジックをそのまま拡張できます。
ROADMAP §13 が「marker dict / class hierarchy を避ける」と書いている
方針とも整合します。
...
契約を再エンジニアリングしないで済ませるためです（ROADMAP A2）。
...
Clojure 仕様で、`(let [x 1] (recur ...))` は `let` を貫通せず外側の
`fn*` / `loop*` の target を見にいきます。
...
なるため、評価器の形が見えてから 3.11 で codegen 寄りに入れる、と
判断しました。
...
ポイント:
- `(try 1 ...)` の body は constant `1` であって `(do 1)` ではない
...
syntax_error を返します。tail-position は 3.9 では問わないものの、
...
専用タグにすると `Value.tag() == .ex_info` の **1 cmp** で識別できます。
marker key map は dispatch のたびに map lookup が走るので、本リポ
ジトリは ROADMAP §13 の「marker dict / class hierarchy を避ける」
方針に沿わせるだけで自然と高速化されます。
...
した最初の例で、決定権限が小規模変更については task 側にある実例です。
...
lifetime を持ち、ExInfo の回収と無関係になります。GC trace の観点でも
独立した root を持つので安全です。
...
正しい例で、ROADMAP P10 の「制御フローに error を使ってよい」方針に
合致します。
...
ひと種類** だけが `Value.tag() == .ex_info` と一致するルールで、
...
顕在化しなかった** バグで、サーベイ段階では気づけませんでした。
...
closure を参照するからです。lifetime が異なるので別 allocator に
乗せる必要があります。
...
`:handled` です。catch がマッチした場合でも finally は走り、戻り値は
catch の結果になります。
...
`macro_transforms.registerInto` で登録するだけで、ユーザ入力を待ち
ます。Phase 3 に入って `defn` や `not` を提供したくなったとき、初めて
**起動時に評価される Clojure ソース** が必要になります。
...
未実装で、3.13 / 3.14 の defn macro 化と並行して整備されます。
...
より `cljw` 単体配布で source tree が無くても起動します。v1 が
...
を fallback / ドキュメントの層として運用するための前提です。
...
metadata map も multi-arity も持ちません。Phase 4+ で user-defined
...
バグになります。Clojure 慣用の暗黙 `do` をマクロ展開時点で展開して
おくのが筋の通った設計です。
...
で表示します。Phase-3 exit smoke のテスト期待値として、このまま観察
できます。
...
「primitive → macro → bootstrap」の順序を守るべきです。
...
value not yet supported (Phase 3+)" です。一方 ROADMAP §9 phase table
の上位行は `(defn f [x] (+ x 1)) (f 2) → 3; try/catch works` という
高レベル文面で、map リテラルを要求していません。**§9 phase table の
高レベル行と Phase 5 の collection scope は内部整合していて、§9.5 /
3.14 の詳細行だけが両方と矛盾していた** という drafting overreach
です。
...
- **D-A**: Phase 3 の exit smoke は `(ex-info "boom" 0)` を使う（`{}`
  ではなく整数の placeholder）。`ex-info` の `data` slot は polymorphic
  で nil 以外なら何でも通るため、try / throw / catch + ex-info の
  round-trip 意味論を検証する目的は不変です。
- **D-B**: map リテラル対応は Phase 5（HAMT / persistent map と
  まとめて）に置く。Phase 3 で空マップだけのスタブを置くのは P4
...
責務を分けています。`{}` を一切引かず、ADR 0002 へのパスだけを
コメントの根拠に残します。
...
2. ADR を `.dev/decisions/NNNN-<slug>.md` に起票（旧文面・新文面・
   ずれていた理由）
...
として残す」と決めた選択が初めて意味を持つ段階で、Zig fast-path と
user macro が共存する dispatcher の優先順位制御がテーマになります。
